...
def main(cfg: OmegaConf) -> None:
    ...
    run_dir = os.getcwd()
    if hasattr(label_transform, "save"):
        label_transform.save(run_dir)
        print(f"[INFO] Saved label transform JSONs to: {run_dir}")
    else:
        print("[WARN] label_transform has no .save() method")
    ...
    if cfg.load_model:
        ...
        # NEW: actually load weights from best_model.pt if it exists
        ckpt_path = Path(cfg.load_model) / "best_model.pt"
        if ckpt_path.is_file():
            trainer.load_checkpoint(str(ckpt_path))
            print(f"[INFO] Loaded checkpoint from {ckpt_path}")
        else:
            print(f"[WARN] No checkpoint found at {ckpt_path}, training from scratch.")
